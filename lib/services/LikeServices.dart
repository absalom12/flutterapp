...
class LikesServices {
  Future<void> addLike(MyGame game, bool status) async {
    try {
      ...
      await userRef.collection('user_Likes').doc(orderId).set({
        'Id': game.appid,
        'name': game.name,
  ...
  Stream<List<MyGame>> getLikesList() {
    final FirebaseAuth auth = FirebaseAuth.instance;
    final User? user = auth.currentUser;
    final String? userId = user?.uid;
    ...
    return userRef.collection('user_Likes').snapshots().map(
